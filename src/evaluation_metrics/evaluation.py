...
# return indexes of events with value POS or UNK
...
# return start and end times of events with value POS or UNK
...
def compute_tp_fp_fn(pred_events_df, ref_events_df):
    ...
    # get POS events in ref and pred
    ...
    # find the best matches between ref and pred events
    ...
    # these are the UNK events that were matched with pred events, they are neither POS nor NEG.
    ...
    # pred = tp + fp, remove matched events with UNK, since they are neither POS nor NEG
    tp = len(m_pos)
...
def evaluate(pred_csv, ref_file_path, team_name, dataset, savepath, metadata=[]):
    ...
    # dataset = VAL or EVAL
    # gt_file_structure[dataset][subset] = [audiofile1, audiofile2, ...]
    gt_file_structure = {}
    gt_file_structure[dataset] = {}
    
    # use audiofile to get subset(=folder) name(HB)
    inv_gt_file_structure = {}
    # get list of subsets(=folders) in ref_file_path(just filenames not full path)
    ...
    # key = audiofilename, value = df of events for this audiofile
    ...
    for audiofilename in list(pred_events_by_audiofile.keys()):
        ...
        #Remove the 5 shots from GT: (everything before the 5th shot is removed)

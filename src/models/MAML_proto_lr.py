...
class MAML_proto_lr(BaseModel):
    ...
    def __init__(self, config):
        ...
        super(MAML_proto_lr, self).__init__(config)
        self.config = config
    ...
    def inner_loop(self, support_data, support_feature, support_label, mode = 'train'):
        ...
        for i in range(self.config.train.inner_step):
            ...
            loss, preds, acc = self.feed_forward(output_weight, output_bias, support_data, support_label, mode = mode)
            ...
            if self.approx:
                ...
                grad_head = [ g.detach()  for g in grad_head ]
                
            model_parameters_name = [name for name, weight in self.feature_extractor.named_parameters() if weight.requires_grad]
            names_grads_copy = dict(zip(model_parameters_name, grad))
            
            fast_parameters = []
                
            for k, (name, weight) in enumerate(self.feature_extractor.named_parameters()):
                # print(self.inner_loop_optimizer.names_learning_rates_dict[name.replace(".", "-")][i])

                if weight.fast is None:
                    if weight.requires_grad:
                        weight.fast = weight.data - self.inner_loop_optimizer.names_learning_rates_dict[name.replace(".", "-")][i]* names_grads_copy[name]
                        # print(name)
                    else:
                        weight.fast = weight.data
                else:
                    if weight.requires_grad:
                        weight.fast = weight.fast - self.inner_loop_optimizer.names_learning_rates_dict[name.replace(".", "-")][i]* names_grads_copy[name]
                        # print(name)
                    else:
    ...
    def feed_forward(self, output_weight, output_bias, data, labels, mode):
    ...
    def feed_forward_test(self, output_weight, output_bias, data):
    ...
    def outer_loop(self, data_loader, mode = 'train', opt = None):
        for i, task_batch in tqdm(enumerate(data_loader)):
            ...
            for task in task_batch:
                ...
                loss, report, acc = self.feed_forward(output_weight, output_bias, query_data , query_label, mode = 'train')
                # print(report)
                # print('~~~~')
                # print(loss.item())
        ...
        for i in range(1):
            ...
            for i, (pos_sup, neg_sup, query, seg_len, seg_hop, query_start, query_end, label) in enumerate(test_loader):
                ...
                for i in range(5):
                    ...
                    output_weight, output_bias = self.inner_loop(support_data, proto, support_label, mode = 'test')
                    ...
                    for batch in tqdm(query_loader):
                        query_data, _ = batch
                        prob = self.feed_forward_test(output_weight, output_bias, query_data)

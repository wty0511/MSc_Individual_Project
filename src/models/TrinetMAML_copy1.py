...
class TNNMAML(BaseModel):
    ...
    def z_score_normalization(self, tensor):
        mean_vals = tensor.mean(dim=0)
        std_vals = tensor.std(dim=0)    
        normalized_tensor = (tensor - mean_vals) / (std_vals+1e-6)

        return normalized_tensor
    
    def get_representation(self, data):
        normalized_data = self.z_score_normalization(data).cpu().detach().numpy()
        db = DBSCAN(eps=50000, min_samples=5).fit(normalized_data)
        labels = db.labels_
        ...
        for cluster in clusters:
            ...
            cluster_samples = normalized_data[indices]
            cluster_center = np.mean(cluster_samples, axis=0)
            distances = np.linalg.norm(cluster_samples - cluster_center, axis=1)
            # print(distances)
            representative.extend(indices[np.argpartition(distances, kth=2)[:2].tolist()])
        return representative
        

        

    def inner_loop_test(self, support_data, support_label = None, mode = 'train'):
        ...
        for i in range(5):
            ...
            neg_dataset = TensorDataset(neg, torch.zeros(neg.shape[0]))
            neg_loader = DataLoader(neg_dataset, batch_size=self.test_loop_batch_size, shuffle=False)
            neg_feat = []
            with torch.no_grad():
                for batch in neg_loader:
                    data, _ = batch
                    neg_feat.append(self.feature_extractor(data))
            neg_feat = torch.cat(neg_feat, dim=0)
            
            rep_neg = self.get_representation(neg_feat)
            support_data = torch.cat([pos, neg[rep_neg]], dim=0)
            support_label = np.concatenate((np.zeros((pos.shape[0],)), np.ones((len(rep_neg),))))
    ...
    def get_topk_sim(self, pos, neg):
        ...
        k= 5
        _, indices = torch.topk(similarity_scores, k= neg.size(0))
        ...
        for i in range(10):
            ...
            for i, (pos_sup, neg_sup, query, seg_len, seg_hop, query_start, query_end, label) in enumerate(test_loader):
                ...
                for i in range(1):
                    ...
                    self.inner_loop_test(support_data, support_label, mode = 'test')

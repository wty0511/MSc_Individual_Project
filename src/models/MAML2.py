...
class MAML2(BaseModel):
    ...
    def __init__(self, config):
        ...
        super(MAML2, self).__init__(config)
        self.config = config
    ...
    def inner_loop(self, support_data, support_label, mode = 'train'):
        ...
        for i in range(self.config.train.inner_step):
            ...
            loss, preds, acc = self.feed_forward(local_model, support_data, support_label, mode = mode)
        ...
        return local_model, support_feat
    ...
    def feed_forward(self, local_model,  data, labels, mode):
        ...
        for batch in feat_data_loader:
            data, _ = batch
            preds.append(local_model(data))
            
        preds = torch.cat(preds, dim=0)
        pos_num = torch.sum(labels == 0)
        neg_num = torch.sum(labels == 1)
    ...
    def feed_forward_test(self, local_model,  data):
        # Execute a model with given output layer weights and inputs
        local_model.eval()
    ...
    def outer_loop(self, data_loader, mode = 'train', opt = None):
        ...
        for i, task_batch in tqdm(enumerate(data_loader)):
            ...
            for task in task_batch:
                ...
                loss, _, acc = self.feed_forward(local_model,  query_data , query_label, mode = 'train')

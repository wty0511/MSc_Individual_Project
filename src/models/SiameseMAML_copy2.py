    ...
    def inner_loop(self, support_data, support_label = None, mode = 'train'):
        ...
        for i in range(1):
            for batch in dataloader:
                data, lable = batch
                anchor, pos, neg = data
                prob1 = self.feature_extractor(anchor, pos)
                loss = self.bce(prob1, torch.ones((prob1.shape[0],1)).float().to(self.device))
                prob2 = self.feature_extractor(anchor, neg)
                loss += self.ce(prob2, torch.zeros((prob1.shape[0],1)).float().to(self.device))             

...
class ProtoMAML_proxy(BaseModel):
    ...
    def inner_loop(self, support_data, support_feature, support_label, mode = 'train'):
        if mode == 'train':
            loss_func = losses.ProxyNCALoss(num_classes=self.n_way, embedding_size=512,reducer= reducers.MeanReducer()).to(torch.device('cuda'))
        else:
            loss_func = losses.ProxyNCALoss(num_classes=2, embedding_size=512,reducer= reducers.MeanReducer()).to(torch.device('cuda'))
        ...
        # prototypes     = support_feature.mean(0).squeeze().detach().requires_grad_()
        ...
        # loss_func.proxies = nn.parameter.Parameter(prototypes)
        ...
        # local_optim = optim.SGD(list(local_model.parameters())+list(loss_func.parameters()), lr= self.config.train.lr_inner)

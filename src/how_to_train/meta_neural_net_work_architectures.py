...
class MetaConvNormLayerReLU(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, stride, padding, use_bias, cfg, normalization=True,
        ...
        self.use_per_step_bn_statistics = self.config.train.per_step_bn_statistics
        ...
        if self.normalization:
            if self.config.train.norm_layer == "batch_norm":
                self.norm_layer = MetaBatchNormLayer(out.shape[1], track_running_stats=True,
                                                     ...
                                                     cfg=self.config)
            elif self.config.train.norm_laye == "layer_norm":
...
class MetaNormLayerConvReLU(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, stride, padding, use_bias, cfg, normalization=True,
        ...
        self.normalization = normalization
        self.use_per_step_bn_statistics = cfg.per_step_bn_statistics
        self.input_shape = input_shape
        self.config = cfg
        self.num_filters = num_filters
        self.kernel_size = kernel_size
        ...
        if self.normalization:
            if self.config.train.norm_layer == "batch_norm":
                self.norm_layer = MetaBatchNormLayer(self.input_shape[1], track_running_stats=True,
                                                     ...
                                                     use_per_step_bn_statistics=self.use_per_step_bn_statistics,
                                                     cfg=self.config)
            elif self.config.train.norm_layer == "layer_norm":
                self.norm_layer = MetaLayerNormLayer(input_feature_shape=out.shape[1:])

...
class MAMLFewShotClassifierWithHead(nn.Module):
    def __init__(self, cfg):
        ...
        """
        super(MAMLFewShotClassifierWithHead, self).__init__()
        self.config = cfg
        ...
        self.classifier = ConvnetClassifier(cfg = cfg, meta_classifier=True).to(device=self.device)
            ...
            for i, (pos_sup, neg_sup, query, seg_len, seg_hop, query_start, query_end, label) in enumerate(data_batch):
                ...
                for i in range(5):
                    ...
                    for num_step in range(num_steps):
                        ...
                        # print(names_weights_copy['weight'])
                        # print('~~~~')
                        # preds = self.classifier(support_data, num_step=self.config.train.inner_step - 1, training=True, backup_running_statistics=True, output_features=False)
    ...
    def net_forward(self, x, y, weights, backup_running_statistics, training, num_step, test = False):
        ...
        if test:
            loss = F.cross_entropy(preds, y, weight=torch.tensor([neg_num/pos_num, 1.0]).to(self.device))
        else:
            preds = preds
